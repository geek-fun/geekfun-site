...
- **Products index:** one row of six icon-over-name items with 64px icons, no card backgrounds, descriptions, or dividers; 6 columns, 3 at ≤1024px, 2 at ≤640px; hover lifts the icon and tints the name forge amber

The homepage hero is a **venetian-blind 6-column flip** (HeroBlind): the studio content renders as the front layer, visible through the panels' transparent front faces; six equal vertical panels each flip 180° to reveal a project (screenshot backdrop, scrim, logo + name + description). Panels open one at a time, left to right, about 5s apart: the front holds ~6s, the first panel opens, and each next one follows roughly 5s later until all six are open. They rest open ~10s, then close right to left in a quick sweep, and the cycle repeats, pausing on hover/focus. Clicking a closed panel (for example while the front is showing) flips that column open immediately and reveals its image and description; the cycle stays paused while the pointer is over the hero. `prefers-reduced-motion` holds a static front and shows a compact 2-column product list in the stage's place, so motion never hides product content. ≤768px uses that same product list. This replaced the former product carousel and the 2-column products grid.
