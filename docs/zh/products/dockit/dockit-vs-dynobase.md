...
用自然语言告诉代理你想干什么，它能写 PartiQL 查询、读表结构、创建条目、更新记录、删数据，全程一个对话搞定。它知道你的表长什么样（键名、类型、GSI 定义），带着 5 个 DynamoDB 工具，分安全、升高、破坏性三个风险等级。权限模式（询问/自动）让你决定代理能放开手脚到什么程度。支持 12 种 AI 提供商，包括 OpenAI、Anthropic、DeepSeek、Ollama、LM Studio。Dynobase 没这功能。
