...
用自然语言描述需求——DocKit 的代理会写 PartiQL 查询、读表结构、创建条目、更新记录、删除数据，全程在一个对话里完成。它理解你的表结构（键名、类型、GSI 定义），拥有 5 个 DynamoDB 工具，按风险等级（安全/升高/破坏性）分类。权限模式（询问/自动）让你控制代理能做什么。支持 12 种 AI 提供商，包括 OpenAI、Anthropic、DeepSeek、Ollama、LM Studio。Dynobase 没有类似功能。
