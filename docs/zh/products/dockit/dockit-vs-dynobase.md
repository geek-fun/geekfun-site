...
| **DynamoDB Local** | ✅ | ✅ |
| **导出 JSON/CSV** | ✅ | ✅ |
| **Agentic Data Studio** | ✅（OpenAI、Anthropic、DeepSeek 等） | ❌ |
...
用自然语言告诉代理你想干什么，它能写 PartiQL 查询、读表结构、创建条目、更新记录、删数据，全程一个对话搞定。它知道你的表长什么样（键名、类型、GSI 定义），带着 DynamoDB 工具，分安全、升高、破坏性三个风险等级。权限模式（询问/自动）让你决定代理能放开手脚到什么程度。支持 12 种 AI 提供商，包括 OpenAI、Anthropic、DeepSeek、Ollama、LM Studio。Dynobase 没这功能。
...
- ✅ 想要**免费开源**，能看源码
- ✅ 要同时管**多种数据库**（MongoDB、DynamoDB + Elasticsearch/OpenSearch/EasySearch）
- ✅ 讨厌云端依赖，就想用个**本地纯工具**
- ✅ 对**数据隐私**极度敏感
- ✅ 个人开发者或小团队（能省则省）
- ✅ 需要 **Agentic Data Studio** 做自然语言数据库交互
...
→ **[DocKit 完整功能概览](/products/dockit/)** · [DynamoDB GUI 客户端页面](/products/dockit/dynamodb-gui-client) · [DynamoDB GUI 深度解析](/zh/products/dockit/dynamodb-gui-client)
