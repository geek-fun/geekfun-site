...
侧边栏 AI 助手和 Agentic Data Studio 能读到你的连接、索引映射、集群状态和查询历史。你提出需求时，它们从集群拉实时上下文，生成 DSL，然后通过验证过的工具直接在 Elasticsearch 上执行。

例如"查找过去一小时最慢的查询"——Agent 读集群元数据，构造查询，执行，返回结果。读操作自动执行，写操作需要你确认。凭据不会发给 LLM。
