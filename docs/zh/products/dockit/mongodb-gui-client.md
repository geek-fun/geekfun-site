...
DocKit 填补了这一空白：提供 VS Code 级别的编辑器体验（Monaco）、能理解集合结构的 AI Agent，以及在同一应用中同时管理 MongoDB、Elasticsearch、OpenSearch 和 DynamoDB 的能力。

### MongoDB AI Agent

DocKit 内置 **Agentic Data Studio**，提供 MongoDB 专属工具。用自然语言描述您的需求——例如"查找上个月注册的所有用户"或"按地区显示平均订单金额"——代理将自动编写查询、检查模式并返回结果。支持 `OpenAI`、`Anthropic`、`DeepSeek` 等多种 AI 提供商。您的数据始终保留在本地。
...
**文档浏览** — 分页浏览，支持内联 CRUD。可配置每页显示条数，在结果表中直接创建、编辑和删除文档，语法高亮 JSON 查看文档结构。

**集合管理** — 查看和管理索引（类型、状态、键信息）、文档计数、数据大小和集合元数据。支持创建、修改、清空和删除集合，无需使用 `mongosh`。

### AI Agent — Agentic Data Studio

这不是一个挂了个聊天框的 LLM。侧边栏 AI 助手和 Agentic Data Studio 都基于 Agentic 架构构建——它们知道你的连接、Schema、索引映射和查询历史。当你提出需求时，它们不靠猜——而是从你的集群拉取实时上下文，生成查询，并通过验证过的工具直接在你的数据库上执行。

例如"查找上个月注册但未验证邮箱的用户"——Agent 读取集合 Schema，构造查询，执行并返回结果。又如"在 `created_at` 字段上创建索引"——它生成命令，在你确认后执行。

Agent 使用带安全门的工具系统：读操作自动执行，写操作（更新、删除、创建索引）需要你明确确认。你的凭据永远不会暴露给 LLM。
