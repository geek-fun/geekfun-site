...
DocKit 还提供大多数 DynamoDB 客户端所没有的功能：**了解您表结构的 AI Agent**。用自然语言描述查询需求——Agent 读取你的表结构，生成 PartiQL，并通过验证过的带安全门的工具在你的表上直接执行。
...
这不是一个挂了个聊天框的 LLM。侧边栏 AI 助手和 Agentic Data Studio 都基于 Agentic 架构构建——它们知道你的连接、表结构、索引和查询历史。当你提出需求时，它们不靠猜——而是从你的 DynamoDB 连接拉取实时上下文，生成 PartiQL，并通过验证过的工具直接在你的表上执行。

例如"查找上周超过 500 美元的订单"——Agent 读取表结构，构造查询，执行并返回结果。Agent 使用带安全门的工具系统：读操作自动执行，写操作（更新、删除）需要你明确确认。你的凭据永远不会暴露给 LLM。
