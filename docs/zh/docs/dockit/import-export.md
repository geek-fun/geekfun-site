...
description: "使用 DocKit 从 MongoDB、Elasticsearch、OpenSearch、DynamoDB 导入导出数据的分步指南。支持 JSON、CSV、批量格式。迁移 NoSQL 数据库、备份数据、测试环境填充，桌面 GUI 客户端完整工作流程。"
head:
  - - meta
    - name: keywords
...
DocKit 支持以 JSON、CSV 和 Elasticsearch Bulk 格式从 MongoDB、Elasticsearch、OpenSearch 和 DynamoDB 导入和导出数据。本指南涵盖两种操作的完整工作流程。
...
- 已配置到 MongoDB、Elasticsearch、OpenSearch 或 DynamoDB 实例的连接。请参阅[连接到服务器指南](/zh/docs/dockit/connect-to-server)。
...
DocKit 现在采用**多步骤导出向导**，提供引导式的导出体验：

**第 1 步 — 数据源与范围**

选择连接、数据库类型以及要导出的具体索引/集合/表。对于 MongoDB，还可以指定过滤查询（JSON 格式）和排序条件。

**第 2 步 — 模式与结构**（可选）

预览源数据的模式。DocKit 会显示字段名称、类型和示例值。您可以选择包含或排除特定字段。

**第 3 步 — 目标与输出**

选择导出格式和目标位置：

- **格式**：JSON、JSONL（JSON Lines）或 CSV
- **输出文件**：浏览选择目标文件夹和文件名
- **美化 JSON**：美化 JSON 输出格式（仅 JSON）
- **包含元数据**：导出一份附带的 `metadata.json` 文件，包含模式信息、导出时间戳和行数

**第 4 步 — 执行**

查看摘要信息（目标索引、预估文档数、预估文件大小），然后点击导出。任务将在**后台**运行，您可以继续其他操作。

## 导入向导

DocKit 的**导入向导**同样采用多步骤设计：

**第 1 步 — 选择数据**

选择数据文件（JSON、JSONL、CSV），也可以选择之前导出时生成的元数据文件。DocKit 会自动检测格式。

**第 2 步 — 模式与结构**

DocKit 会并排显示源模式（字段名称、类型、示例值）与目标模式。字段状态分为：

- **匹配** — 源和目标中都存在的字段（自动映射）
- **新增** — 源中有但目标中没有的字段（将被添加）
- **排除** — 选择在导入时跳过的字段

您可以覆盖目标字段的类型（例如将 String 改为 Number）。

**第 3 步 — 目标与输出**

选择目标数据库和集合/表。对于新建的集合，可以配置：

- MongoDB：集合名称
- Elasticsearch：分片数量、副本数量
- DynamoDB：分区键、排序键、计费模式、容量单位

**第 4 步 — 执行**

选择导入策略：

- **追加**：将数据添加到现有记录中（跳过重复项）
- **替换**：使用 upsert 方式覆盖现有记录

点击导入。任务将作为**后台任务**运行 — 在任务管理器面板中跟踪进度。

## 后台任务

导入和导出都以**后台任务**方式运行。您可以：

- 在**任务管理器**（右侧边栏面板）中监控进度
- 查看实时状态：待处理 → 运行中 → 已完成/失败
- 查看详情：已处理的文档数、遇到的错误、已用时间
- 在任务执行期间正常使用 DocKit

## MongoDB 支持

导入和导出功能同样适用于 MongoDB 集合：

- **导出**：支持过滤查询、字段选择和排序条件导出文档
- **导入**：支持将 JSON/CSV 文件导入 MongoDB 集合，提供追加和替换两种策略
- 每批处理 500 个文档

## 元数据导出

勾选**包含元数据**后，DocKit 会在数据文件旁导出一份 `metadata.json` 文件。该文件包含：
