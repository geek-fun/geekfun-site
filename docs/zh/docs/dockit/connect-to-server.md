...
常见的连接错误：`Connection refused` 时检查主机和端口；`Unauthorized` 时核对凭证；`SSL handshake failed` 时检查证书配置；`Index not found` 时提供有效的索引名。

## 保存和管理连接

点击 **确认** 保存连接。连接会加密存储在本地，在侧边栏中显示以便快速访问，应用重启后依然保留。

你可以按名称、类型或日期对连接排序，按名称搜索筛选，右键编辑或删除，点击侧边栏中的连接即可快速切换。

## 安全最佳实践

生产环境优先使用 API Key——便于轮换和审计。非本地开发环境始终开启 SSL。只授予工作流实际需要的索引权限。为开发、预发布和生产环境分别设置不同的连接和凭证。定期轮换密钥和密码。
