---
title: DocKit Agentic Data Studio — AI 代理数据库工作台
...
head:
  - - meta
    - name: keywords
      content: DocKit Agentic Data Studio, AI 数据库代理, 自然语言数据库查询, AI 助手, 代理循环, 工具执行, 数据库 AI 代理, OpenAI, Anthropic, DeepSeek, Ollama, DocKit AI
  - - link
    - rel: canonical
...
DocKit 的 Agentic Data Studio 是一个与您的数据库通过自然语言交互的 AI 代理。描述您的需求——代理会编写查询、检查表结构、更新文档、删除记录、创建索引并返回结果——全部在单个对话中完成。

侧边栏助手提供同样的 AI 能力，以轻量级聊天面板的形式，方便您在编辑器中快速提问和生成查询，无需切换视图。

## 代理能做什么

代理拥有 **28 个工具**，覆盖所有支持的数据库：

**Elasticsearch / OpenSearch / EasySearch**（16 个工具）：
- 搜索、获取、索引、更新和删除文档
- 列出索引、获取映射、创建/删除索引
...
**DynamoDB**（5 个工具）：
- 执行 PartiQL SELECT、INSERT、UPDATE、DELETE
- 描述和列出表
...
侧边栏助手是一个轻量级聊天面板，可在应用的任意位置访问。它与 Data Studio 共享相同的提供商和模型，但以单轮 Q&A 接口的方式运行：
...
**凭证安全**：连接密钥存储在您的系统密钥链中，永远不会暴露给 LLM。代理使用凭证安全架构——工具通过 Rust 后端执行，而非通过 AI 提供商。

**无遥测**：DocKit 不会回传任何数据。查询内容、凭据或分析数据都不会离开您的电脑。支持离线环境。
