...
DocKit 有两种权限模式。**询问模式**（默认）会在每个非只读工具执行前提示允许或拒绝——适合生产环境。**自动模式**在会话拥有足够权限时自动执行工具，探索时更高效。

每个工具按风险分类：**安全**工具如读取查询和列出索引，从不要求确认。**升高**工具如创建或更新文档，仅在询问模式下要求确认。**破坏性**工具如删除文档或索引，无论处于哪种模式都始终要求确认。

每个数据库源都有独立的读取、创建、更新、删除权限。可以按源设置，也可以从会话级别继承。还可以设置确认规则，自动允许安全或重复操作，或自动拒绝危险工具。
...
代理会显示当前阶段：`准备中 → 迭代中 → 等待模型 → 压缩中 → 完成`。每次工具调用都会显示名称、参数、耗时和结果，展开可查看完整的输出内容。
