...
DocKit 支持以 JSON、CSV 和 JSONL 格式从 MongoDB、Elasticsearch、OpenSearch 和 DynamoDB 导入和导出数据。本指南涵盖完整的工作流程。

## [查询历史](/zh/docs/dockit/query-history)

永不丢失查询记录。DocKit 自动记录 MongoDB、Elasticsearch 和 DynamoDB 的每个已执行查询，让您可以重新运行、复制并将历史查询添加回编辑器。

## [DynamoDB 管理与指标](/zh/docs/dockit/dynamodb-manage-metrics)

DocKit 允许您管理 DynamoDB 表并监控关键指标 — 表大小、项目数量、读/写容量 — 全部通过简洁的桌面 GUI 完成。
