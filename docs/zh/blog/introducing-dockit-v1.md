...
然后是状态丢失的问题。在新浏览器标签页打开 Dashboard？你的筛选条件直接没了——2024 年 Kibana GitHub issue #188914 记录了这个行为。在 Discover 标签页之间切换但没执行查询？草稿直接消失（#246896）。离开电脑几分钟？Session 过期，回到登录页，你正在写的东西全没了。"能不能用弹窗登录而不是跳转到登录页？"一位 Kibana 开发者在 issue #106235 里问。这个 issue 至今还是 Open 的。

需要同时访问多个集群——staging、生产、本地开发？Kibana 的回答是：开多个 Kibana 实例。他们明确关掉了多集群支持的功能请求（#25183），回复是"我们的长期目标是一个 Elasticsearch 集群对应一个 Kibana"。意思就是一个浏览器标签页对应一个环境，各自登录，各自有状态，各自有丢失工作的方式。
...
DynamoDB 呢？AWS 控制台能应付简单查看。但要在上面排查线上问题？就一个标签页：想同时看两张表？准备好点浏览器的后退按钮吧。在 AWS 账号之间切换？"填表填到一半，Session 决定刷新 Token，你输入的所有东西都没了……每一天都这样，"一位开发者在 Reddit 上说。离开电脑几分钟？"笔记本休眠几分钟后醒来，看到错误提示说登录已过期。我们公司登录 AWS 要整整 3 分钟，白白登录太痛苦了。"
