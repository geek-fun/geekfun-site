...
浏览器端的数据库工具有着通病——太重、丢状态、管不了多集群。Kibana、OpenSearch Dashboards、AWS 控制台，我们都用过，全都一个样。

Kibana 要容器，启动 30 秒，吃 500MB 内存。自动补全能打满集群每个 warm 节点的 CPU。新标签页打开 Dashboard，筛选条件直接没了（Kibana #188914，至今未修）。离开几分钟，Session 过期，回到登录页，全丢了（#106235，也还开着）。要同时访问 staging 和生产？开两个 Kibana 实例，两套登录，两套状态各自丢失。多集群支持的功能请求（#25183）被他们关了，理由是"我们的长期目标是一个 ES 集群对应一个 Kibana"。

AWS 控制台查个数据没问题。要排查线上问题？单标签页，没法同时看两张表，Session 刷新还会清掉你正在填的表单。

于是我们做了 DocKit。基于 Tauri 的原生桌面应用。安装包不到 10MB，两秒启动，不需要 Docker 和浏览器。查询就是本地文件——切标签页不会丢。历史自动记录：每个连接 500 条，没有 Session 过期。多个集群在一个窗口，点一下切换。DynamoDB、Elasticsearch、OpenSearch 三种引擎同一界面，同一套编辑器和快捷键。
