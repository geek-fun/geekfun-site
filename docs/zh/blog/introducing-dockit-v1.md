...
**不必要的臃肿。** Kibana 是完整的可视化平台，带仪表盘、监控和告警——但你只需要跑查询。这份"全家桶"带来的是容器、30 秒启动、500MB 内存。光是自动补全就能打满 warm 节点的 CPU。DocKit 首先是查询编辑器。基于 Tauri 的原生桌面应用。不到 10MB，两秒启动。
...
**集群与客户端强绑定。** Kibana 一个实例只能连一个 ES 集群。OpenSearch Dashboards 同理。AWS 控制台把你锁在 DynamoDB 单标签页里，Session 还有倒计时。要同时看 staging 和生产？开两个 Kibana 实例，两套登录，两套状态各自丢失。他们关了多集群请求（#25183），理由是"长期目标是一个 ES 集群对应一个 Kibana"。换个数据库？换套工具。三个引擎，三个客户端，三个地方丢工作——你的机器还得全跑着。DocKit 在一个窗口里连接多个集群。DynamoDB、Elasticsearch、OpenSearch，同一个编辑器，同一套快捷键。点一下切换，不用新实例，不用重复登录。
