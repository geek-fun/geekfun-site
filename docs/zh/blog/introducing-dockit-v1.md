...
**太重。** Kibana 要容器，启动 30 秒，吃 500MB 内存。自动补全能打满集群每个 warm 节点的 CPU。DocKit 是基于 Tauri 的原生桌面应用。安装包不到 10MB，两秒启动。不需要 Docker 和浏览器。

**丢状态。** 新标签页打开 Dashboard，筛选条件直接没了（Kibana #188914，至今未修）。离开几分钟，Session 过期，回到登录页，全丢了（#106235，也还开着）。DocKit 一切保存在本地。查询就是文件系统上的文件。历史自动记录：每个连接 500 条，没有 Session 过期。

**管不了多集群。** 要同时访问 staging 和生产？Kibana 的回答：开两个实例，两套登录，两套状态各自丢失。多集群支持的功能请求（#25183）被他们关了，理由是"长期目标是一个 ES 集群对应一个 Kibana"。DocKit 在一个窗口里连接多个集群。点一下切换。同一套编辑器和快捷键。

**三种数据库三套工具。** DynamoDB、Elasticsearch、OpenSearch——三个控制台，三种查询语言，三个地方可能丢工作。AWS 控制台就一个标签页，Session 刷新还会清掉你正在填的表单。DocKit 把三种引擎放在一个界面里。同一个编辑器，同一套快捷键，处处一致。
