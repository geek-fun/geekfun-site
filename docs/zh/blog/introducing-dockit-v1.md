...
**你要的是查询，不是全家桶。** Kibana 是完整的可视化平台，带仪表盘、监控和告警——但你只需要跑查询。这份"全家桶"带来的是容器、30 秒启动、500MB 内存。光是自动补全就能打满 warm 节点的 CPU。DocKit 首先是查询编辑器。基于 Tauri 的原生桌面应用。不到 10MB，两秒启动。

**写到一半的东西没了。** 新标签页打开 Dashboard，筛选条件直接没了（Kibana #188914，至今未修）。离开几分钟，Session 过期，回到登录页，全丢了（#106235，也还开着）。不是你忘了保存，是工具替你忘了。DocKit 一切保存在本地。查询就是文件系统上的文件。历史自动记录：每个连接 500 条，没有 Session 过期。

**切不了集群。** 要同时看 staging 和生产？Kibana 的回答：开两个实例，两套登录，两套状态各自丢失。多集群支持的功能请求（#25183）被他们关了，理由是"长期目标是一个 ES 集群对应一个 Kibana"。DocKit 在一个窗口里连接多个集群。点一下切换。同一套编辑器和快捷键。

**换引擎就得换工具。** DynamoDB、Elasticsearch、OpenSearch——每个引擎有自己的工具，自己的查询语言，自己的丢状态方式。AWS 控制台单标签页、单表查看，Session 刷新还会清掉你正在填的表单。DocKit 把三种引擎放在一个界面里。同一个编辑器，同一套快捷键，处处一致。
