...
浏览器端的数据库工具有着通病。Kibana、OpenSearch Dashboards、AWS 控制台——它们都跑在浏览器标签页里，而标签页根本不是认真干活的地方。

**太重。** Kibana 需要容器，启动要 30 秒，吃 500MB 内存。它的自动补全甚至能把集群里每个 warm 节点的 CPU 打满。DocKit 是基于 Tauri 的原生桌面应用。安装包不到 10MB，两秒启动。没有 Docker，没有浏览器，没有云。

**丢状态。** 在新标签页打开 Dashboard——筛选条件没了。切 Discover 标签页——查询草稿消失了。离开几分钟——Session 过期，回到登录页，全没了。这些都是 Kibana 已记录但至今未修的 bug（#188914、#246896）。DocKit 一切保存在本地。查询就是文件系统上的文件。历史自动记录——每个连接 500 条，不用点保存，没有会话过期。

**管不了多集群。** Kibana 明确拒绝了多集群支持的功能请求（#25183）——一个 Kibana 对应一个 Elasticsearch 集群，就这样。意味着一个浏览器标签页对应一个环境，各自登录。DocKit 在一个窗口里连接多个集群。Staging、生产、本地，点一下就切换。同一套编辑器和快捷键。

**三种数据库三套工具。** DynamoDB、Elasticsearch、OpenSearch——三个控制台，三种查询语言，三个地方可能丢工作。DocKit 把三种引擎放在一个界面里。同一个编辑器，同一套快捷键，处处一致。
