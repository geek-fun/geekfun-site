...
Amazon DynamoDB 于 2012 年推出，如今覆盖了游戏、金融科技、物联网、无服务器后端等各个领域。围绕它成长出了一个工具生态：GUI 客户端、本地模拟器、ORM、监控面板。Dynobase、NoSQL Workbench、DynamoDB Admin 等等。
...
| 工具 | 价格 | 平台 | 最适合 | 开源 |
|------|-------|----------|----------|-------------|
| DocKit | 免费 | Mac、Win、Linux | 团队、预算有限的开发者 | ✅ |
| Dynobase | $12-30/月 | Mac、Win、Linux | 有预算的个人和团队 | ❌ |
| NoSQL Workbench | 免费 | Mac、Win、Linux | AWS 中心化工作流 | ❌ |
...
**功能（活跃时）：** 精美 UI、可视化查询构建器、云同步、团队协作、导出/导入、DynamoDB Streams 监控。

活着的时候：好 UI、好的文档、云同步。
现在：可能缺失新 DynamoDB 功能、无更新、闭源、仅 DynamoDB、无 AI。
...
目前最好的付费选项。$199 一次性买断。12 个月发了 14 个版本——开发很活跃。

**功能：** SSO 支持、多标签查询、DynamoDB SQL 控制台、导入导出（JSON、CSV、S3）、跨账户支持、请求链、Git 原生查询共享。

**价格：** $199 一次性。试用期外无免费版。

**优点：** 非常活跃、SSO 好用、多标签切换不刷新、SQL 控制台很方便。
**缺点：** 无免费版、仅 DynamoDB、不支持多引擎。

**最适合：** 想要精美付费工具且不需要 Elasticsearch/OpenSearch 的专业人士。
...
**功能：** 零安装浏览器访问、表管理、IAM/CloudWatch 集成、基础查询。

**活跃度：** AWS 持续更新。

**优点：** 到处能用、始终最新。
**缺点：** 慢。关掉标签页查询就没了。需要联网。每页限制 1,000 条。偶尔查一下还行，日常用很痛苦。

**最适合：** 快速查询和管理任务，手边没装其他工具时用。
...
**活跃度：** 1,471 GitHub stars。最近提交 2026 年 4 月 9 日。
...
- **预算为零？** DocKit 或 NoSQL Workbench，看你是需要建模 schema 还是跑查询。
- **想要最好看的 UI 且不介意付费？** Dynomate 是活跃的付费选择。
- **只是偶尔检查一下？** AWS 控制台能用。慢是慢，但能用。
- **只用 DynamoDB Local？** DynamoDB Admin 是最轻的选择。
...
可以。DocKit 做日常查询、NoSQL Workbench 设计 schema、AWS 控制台做管理任务。
